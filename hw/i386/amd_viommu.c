...
#define SUPPORTED_EFR          (FEATURE_GT | FEATURE_PPR | \
                                FEATURE_GIO | FEATURE_GLX_2LEVEL | \
                                FEATURE_PASMAX_16)
...
static bool amdvi_get_hw_info(AMDVIState *s, IOMMUFDDevice *idev)
{
    ...
    if ((s->hwinfo.efr & ~(FEATURE_GATS_MASK << FEATURE_GATS_SHIFT)) != SUPPORTED_EFR) {
        error_report("Unable to support AMD IOMMU feature (efr=%#llx)(%#llx)!!", s->hwinfo.efr, SUPPORTED_EFR);
        s->hwinfo.efr = 0;
        return false;
    }

    if ((s->hwinfo.efr & (FEATURE_GATS_MASK << FEATURE_GATS_SHIFT)) > FEATURE_GATS_5LEVEL) {
        error_report("Unable to support AMD IOMMU feature GATS (efr=%#llx)(%#llx)!!", s->hwinfo.efr, SUPPORTED_EFR);
	return false;
    }
