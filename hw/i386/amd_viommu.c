...
static int amd_viommu_mmap_vf_mmio(AMDVIState *s)
{
    ...
    uint64_t baseaddr = AMDVI_BASE_ADDR + (x86_iommu->index * AMD_VIOMMU_MMIO_SIZE);
...
static void amdvi_vdevice_viommu_setup(AMDVIState *s, AMDIOMMUFDDevice *amd_idev)
{
    ...
    pci_for_each_bus_depth_first(s->primary_bus, amdvi_walk_bus_setup, NULL, amd_idev);
     ...
     */
     if (s->hiod_hash) {
        ...
        while (g_hash_table_iter_next(&hiod_it, (void **)&key, (void **)&hiod)) {
            ...
            if (!host_iommu_device_iommufd_attach_hwpt(idev, idev->ioas_id, &errp)) {
                error_free(errp);
                continue;
            }
            s->hwpt_cnt--;
...
static void amd_viommu_realize(DeviceState *dev, Error **errp)
{
    ...
    /* set up MMIO */
    memory_region_init_io(&s->mr_mmio, OBJECT(s),
                          &mmio_mem_ops, s, "amdvi-mmio",
                          0x2000);

    sysbus_init_mmio(SYS_BUS_DEVICE(s), &s->mr_mmio);
    sysbus_mmio_map(SYS_BUS_DEVICE(s), 0,
                    AMDVI_BASE_ADDR + (x86_iommu->index * AMD_VIOMMU_MMIO_SIZE));
